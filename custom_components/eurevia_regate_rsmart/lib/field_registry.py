...
TERMINAL_FIELD_SPECS: tuple[FieldSensorSpec, ...] = (
    ...
    FieldSensorSpec(
        ...
        "stp_af",
        "terminal_stp_af",
        value_fn=lambda s: as_int(s.get("Stp_AF")),
    ),
    FieldSensorSpec(
        "Fan_E",
        "fan_e",
        "terminal_fan_e",
        unit=PERCENTAGE,
        value_fn=lambda s: as_int(s.get("Fan_E")),
    ),
    FieldSensorSpec(
        "Fan_N",
        "fan_n",
        "terminal_fan_n",
        unit=PERCENTAGE,
        value_fn=lambda s: as_int(s.get("Fan_N")),
    ),
    FieldSensorSpec(
        "Fan_S",
        "fan_s",
        "terminal_fan_s",
        unit=PERCENTAGE,
        value_fn=lambda s: as_int(s.get("Fan_S")),
    ),
    FieldSensorSpec(
        "Fan_Timer",
        "fan_timer",
        "terminal_fan_timer",
        unit=UnitOfTime.MINUTES,
        value_fn=lambda s: as_int(s.get("Fan_Timer")),
    ),
    FieldSensorSpec(
        "Operating_authorization",
        "operating_authorization",
        "terminal_operating_authorization",
        value_fn=lambda s: as_int(s.get("Operating_authorization")),
    ),
    FieldSensorSpec(
        "Operating_Priority",
        "operating_priority",
        "terminal_operating_priority",
        value_fn=lambda s: as_int(s.get("Operating_Priority")),
    ),
    FieldSensorSpec(
        "Absence_Detection_Mode",
        "absence_detection_mode",
        "terminal_absence_detection_mode",
        value_fn=lambda s: as_int(s.get("Absence_Detection_Mode")),
    ),
    FieldSensorSpec(
        "Absence_Detection_Timer",
        "absence_detection_timer",
        "terminal_absence_detection_timer",
        unit=UnitOfTime.MINUTES,
        value_fn=lambda s: as_int(s.get("Absence_Detection_Timer")),
    ),
    FieldSensorSpec(
        "Presence_Detection_Timer",
        "presence_detection_timer",
        "terminal_presence_detection_timer",
        unit=UnitOfTime.MINUTES,
        value_fn=lambda s: as_int(s.get("Presence_Detection_Timer")),
    ),
    FieldSensorSpec(
        "Window_Open_Mode",
        "window_open_mode",
        "terminal_window_open_mode",
        value_fn=lambda s: as_int(s.get("Window_Open_Mode")),
    ),
    FieldSensorSpec(
        "Window_Open_Timer",
        "window_open_timer",
        "terminal_window_open_timer",
        unit=UnitOfTime.MINUTES,
        value_fn=lambda s: as_int(s.get("Window_Open_Timer")),
    ),
    FieldSensorSpec(
        "Window_Close_Timer",
        "window_close_timer",
        "terminal_window_close_timer",
        unit=UnitOfTime.MINUTES,
        value_fn=lambda s: as_int(s.get("Window_Close_Timer")),
    ),
    FieldSensorSpec(
        "Valve_Type",
        "valve_type",
        "terminal_valve_type",
        value_fn=lambda s: as_int(s.get("Valve_Type")),
    ),
    FieldSensorSpec(
        "PID_Disabled_OP",
        "pid_disabled_op_cfg",
        "terminal_pid_disabled_op_cfg",
        value_fn=lambda s: as_int(s.get("PID_Disabled_OP")),
    ),
    FieldSensorSpec(
        "PID_Integral_Default",
        "pid_integral_default_cfg",
        "terminal_pid_integral_default_cfg",
        value_fn=lambda s: as_int(s.get("PID_Integral_Default")),
    ),
    FieldSensorSpec(
        "PID_Max_Interval",
        "pid_max_interval_cfg",
        "terminal_pid_max_interval_cfg",
        value_fn=lambda s: as_int(s.get("PID_Max_Interval")),
    ),
    FieldSensorSpec(
        "PID_Prop_Band",
        "pid_prop_band_cfg",
        "terminal_pid_prop_band_cfg",
        value_fn=lambda s: as_float(s.get("PID_Prop_Band")),
    ),
    FieldSensorSpec(
        "PID_Smooth_Factor",
        "pid_smooth_factor_cfg",
        "terminal_pid_smooth_factor_cfg",
        value_fn=lambda s: as_float(s.get("PID_Smooth_Factor")),
    ),
)
